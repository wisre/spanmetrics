...
func newProcessor(logger *zap.Logger, config config.Processor, nextConsumer consumer.Traces) (*processorImp, error) {
	...
	return &processorImp{
		...
		config:                *pConfig,
		startTime:             time.Now(),
		newItem:               make(chan ptrace.Traces, 2000),
...
func (p *processorImp) ConsumeTraces(ctx context.Context, traces ptrace.Traces) error {
	...
	var err1, err2 error
	select {
	case p.newItem <- traces:
		err1 = nil
	default:
		err1 = fmt.Errorf("trace chan is full")
		p.logger.Error("trace chan is full")
	}
	err2 = p.nextConsumer.ConsumeTraces(ctx, traces)
